...
func NewBlanckImage(cols, rows int, c color.Color) image.Image {
...
func NewPxCanvas(state *apptype.State, config apptype.PxCanvasConfig) *PxCanvas {
	...
	pxCanvas.PixelData = NewBlanckImage(pxCanvas.PxCols, pxCanvas.PxRows, color.NRGBA{128, 128, 128, 255})
...
func (pxCanvas *PxCanvas) NewDrawing(cols, rows int) {
	...
	pixelData := NewBlanckImage(cols, rows, color.NRGBA{128, 128, 128, 255})
